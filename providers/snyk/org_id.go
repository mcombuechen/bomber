...
func getOrgID(client *HttpRequest.Request) (orgID string, err error) {
	...
	if err != nil {
		return orgID, fmt.Errorf("unable to retrieve org ID: %w", err)
	}
	...
	if res.StatusCode() != 200 {
		return orgID, fmt.Errorf("unable to retrieve org ID (status: %s)", res.Response().Status)
	}

	var userInfo selfDocument
	if err = json.Unmarshal(body, &userInfo); err != nil {
		return orgID, fmt.Errorf("unable to retrieve org ID (status: %s): %w", res.Response().Status, err)
	}
