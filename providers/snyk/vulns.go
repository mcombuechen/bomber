...
func getVulnsForPurl(
	...
	issuesURL := fmt.Sprintf("%s/orgs/%s/packages/%s/issues%s", SNYK_URL, orgID, url.QueryEscape(purl), SNYK_API_VERSION)

	res, err := client.Get(issuesURL)
	if err != nil {
		return vulns, fmt.Errorf("could not fetch vulnerabilities (purl: %s): %w", purl, err)
	}
	defer res.Close()

	body, err := res.Body()
	if err != nil {
		return vulns, fmt.Errorf("could not read response body (purl: %s): %w", purl, err)
	}

	if res.StatusCode() != 200 {
		return vulns, fmt.Errorf("failed request while retrieving vulnerabilities (purl: %s, status: %s): %w", purl, res.Response().Status, err)
	}

	var response SnykIssuesDocument
	if err = json.Unmarshal(body, &response); err != nil {
		return vulns, fmt.Errorf("could not parse response (purl: %s): %w", purl, err)
	}
